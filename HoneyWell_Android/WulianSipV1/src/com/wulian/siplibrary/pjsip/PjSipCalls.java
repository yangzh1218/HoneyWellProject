...
public final class PjSipCalls {
	 ...
	 * 
	 * @param session
	 *            The session to update (input/output). Must have a correct call
	 *            id set
	 * @param service
	 *            PjSipService Sip service to retrieve pjsip accounts infos
	 * @throws SameThreadException
	 */
	public static void updateSessionFromPj(SipCallSessionImpl session,
			pjsip_event e, Context context) {
		WulianLog.d(THIS_FILE, "Update call " + session.getCallId());
		...
		if (status == pjsua.PJ_SUCCESS) {
			...
			// Update state here because we have pjsip_event here and can get
			// q.850 state
			if (e != null) {
				// Status code
				...
				WulianLog.d(THIS_FILE, "Last status code is " + status_code);
				// TODO - get comment from q.850 state as well
				String status_text = PjSipService.pjStrToString(pjInfo
						.getLast_status_text());
			...
			session.setSignalisationSecure(pjsua.call_secure_sig_level(session
					.getCallId()));
			String secureInfo = PjSipService.pjStrToString(pjsua
					.call_secure_media_info(session.getCallId()));
			session.setMediaSecureInfo(secureInfo);
			session.setMediaSecure(!TextUtils.isEmpty(secureInfo));
			zrtp_state_info zrtpInfo = pjsua.jzrtp_getInfoFromCall(session
					.getCallId());
			...
			// About video info
			int vidStreamIdx = pjsua.call_get_vid_stream_idx(session
					.getCallId());
			if (vidStreamIdx >= 0) {
				int hasVid = pjsua.call_vid_stream_is_running(
						session.getCallId(), vidStreamIdx,
						pjmedia_dir.PJMEDIA_DIR_DECODING);
			...
			session.setCallState(pjsip_inv_state.PJSIP_INV_STATE_DISCONNECTED
					.swigValue());
		}
	}
	 ...
	 * 
	 * @param session
	 *            the session to copy info to (output)
	 * @param pjCallInfo
	 *            the call info from pjsip
	 * @param service
	 *            PjSipService Sip service to retrieve pjsip accounts infos
	 */
	private static void updateSession(SipCallSessionImpl session,
			pjsua_call_info pjCallInfo, Context context) {
		...
		session.setRemoteContact(PjSipService.pjStrToString(pjCallInfo
				.getRemote_info()));
		...
		session.setConnectStart(SystemClock.elapsedRealtime()
	...
	public static String dumpCallInfo(int callId) {
		return PjSipService.pjStrToString(pjsua.call_dump(callId,
				pjsua.PJ_TRUE, " "));
	}
	 ...
	 * @throws SameThreadException
	 */
	public static String dumpCallSpeedInfo(int callId) {
		return PjSipService.pjStrToString(pjsua.call_dump_speed(callId,
				pjsua.PJ_TRUE, " "));
	}
	 ...
	 * @throws SameThreadException
	 */
	public static String dumpCallNatInfo(int callId) {
		return PjSipService.pjStrToString(pjsua.call_dump_nat(callId,
				pjsua.PJ_TRUE, " "));
	}
