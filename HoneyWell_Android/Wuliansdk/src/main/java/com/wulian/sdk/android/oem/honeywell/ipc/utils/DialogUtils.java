...
import com.wulian.sdk.android.oem.honeywell.ipc.ui.widget.PBWebView;
import com.wulian.sdk.android.oem.honeywell.ipc.utils.wheel.NumericWheelAdapter;
import com.wulian.sdk.android.oem.honeywell.ipc.utils.wheel.WheelView;
...
//			settings.setTextSize(WebSettings.TextSize.SMALLER);
	...
	public static Dialog showBarcodeConfigTipDialog(Context mContext) {
		View layout = LayoutInflater.from(mContext).inflate(
	...
	public static Dialog showCommonDialog(Context mContext, boolean isCancel,
			String tip, CharSequence message, String ok, String cancle,
			final OnClickListener okOnclick) {
		View layout = LayoutInflater.from(mContext).inflate(
	...
	public static Dialog showSetPwdEditDialog(Context mContext,
											  boolean isCancel, String tip, String ok, String cancle,
											  String editHint, String editInfo, final OnClickListener okOnclick) {
		View layout = LayoutInflater.from(mContext).inflate(
				R.layout.custom_common_setpwd_alertdialog, null);
		Dialog dialog = new Dialog(mContext, R.style.alertDialog);
		dialog.setContentView(layout);
		dialog.setCancelable(isCancel);
		dialog.setCanceledOnTouchOutside(false);
	 ...
	 * @param cancle
	 *            dialog cancle键
	 * @param defaultDayTime
	 *            格式是0,1,2,3---->周日，周一，周二，周三，
	 * @param dataSource
	 *            可选时间周期列表
	 * @param okOnclick
	 *            传过来的点击事件，用于捆绑dialog中的ok和cancle两个点击事件
	 * @return
	 */
			...
			public void onClick(View v) {
				if (null != okOnclick) {
					...
					for (int i = 0; i < dataSource.size(); i++) {
						// why lv_info.getCheckedItemPositions().valueAt(i) 值不对
						// fuck
	...
	public static class SceneAdapter extends BaseAdapter {
		...
		private List<Scene.SData> list;
		private Scene.SData tempGridViewItem;
		private LayoutInflater layoutInflater;

		public SceneAdapter(Context context, List<Scene.SData> list) {
			this.list = list;
			layoutInflater = LayoutInflater.from(context);
		}

		public void refreshAdapter(List<Scene.SData> list) {
			if (list == null) {
				return;
			}
			if (this.list != null) {
		...
		@Override
		public Scene.SData getItem(int position) {
	...
	public Dialog showCommonGridViewDialog(final Context mContext,
			boolean isCancel, int defaultIdx,
			final Scene.OnSelectionLisenter mSelectionListener,
	...
	public static Dialog showQueryIdDialog(Context context){
		View layout = LayoutInflater.from(context).inflate(
				R.layout.activity_deviceid_query, null);
	...
	public static void initQueryIdDialog(Dialog dialog, boolean isQuery, View.OnClickListener onClickListener) {
...
//        RelativeLayout rl_query_device_fail = (RelativeLayout) dialog.findViewById(R.id.rl_query_device_fail);
		...
		btn_retry_query_device.setOnClickListener(onClickListener);
	}
}
